...
{
    public class OzoneDa
    {
        public static List<OzoneExt> GetAllOzones(long? ozoneId = null, long? experimentProcessId = null, long? batchProcessId = null)
        {
            DataTable dt;
            ...
            try
            {
                ...
                cmd.CommandText =
                    ...
                    WHERE (c.ozone_id = :cid or :cid is null) and
                        (c.fk_experiment_process = :epid or :epid is null) and
                        (c.fk_batch_process = :bpid or :bpid is null);";

                Db.CreateParameterFunc(cmd, "@cid", ozoneId, NpgsqlDbType.Bigint);
            ...
            List<OzoneExt> list = (from DataRow dr in dt.Rows select CreateObjectExt(dr)).ToList();

            return list;
        }
        public static List<OzoneExt> GetRecentlyUsedOzones(int? researchGroupId = null, long? experimentProcessId = null, long? batchProcessId = null)
        {
            DataTable dt;
            ...
            try
            {
                ...
                cmd.CommandText =
                    @"SELECT max(ozone_id) as ozone_id, max(date_created) as date_created, fk_equipment, e.equipment_name,
time,
comments,
label
                      FROM ozone
                          LEFT JOIN equipment e on ozone.fk_equipment = e.equipment_id
                      GROUP BY fk_equipment, e.equipment_name,
time,
comments,
label
                      ORDER BY max(ozone_id) DESC LIMIT 10;";
        ...
        public static int AddOzone(Ozone ozone, NpgsqlCommand cmd)
        {
            try
            {
                if (cmd != null)
...
:label
                ...
                Db.CreateParameterFunc(cmd, "@epid", ozone.fkExperimentProcess, NpgsqlDbType.Bigint);
                Db.CreateParameterFunc(cmd, "@bpid", ozone.fkBatchProcess, NpgsqlDbType.Bigint);
                Db.CreateParameterFunc(cmd, "@eid", ozone.fkEquipment, NpgsqlDbType.Integer);
                Db.CreateParameterFunc(cmd, "@time", ozone.time, NpgsqlDbType.Double);
                Db.CreateParameterFunc(cmd, "@comments", ozone.comments, NpgsqlDbType.Text);
                Db.CreateParameterFunc(cmd, "@label", ozone.label, NpgsqlDbType.Text);
        ...
        public static int UpdateOzone(Ozone ozone)
        {
            try
            {
                var cmd = Db.CreateCommand();
...
label=:label
                ...
                Db.CreateParameterFunc(cmd, "@cid", ozone.ozoneId, NpgsqlDbType.Bigint);
        ...
        public static Ozone CreateObject(DataRow dr)
        {
        ...
        private static OzoneExt CreateObjectExt(DataRow dr)
        {
            var ozone = CreateObject(dr);

            var ozoneExt = new OzoneExt(ozone)
            {
