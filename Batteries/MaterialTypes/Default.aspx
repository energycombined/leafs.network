<%@ Page Title="Material Types" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="Batteries.MaterialTypes.Default" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <section class="content">
        ...
        <hr />
        <p>
            <asp:HyperLink runat="server" NavigateUrl="Insert" CssClass="btn btn-primary" Text="New material type" />
        </p>
        ...
        <script type="text/javascript">
            ...
            function RefreshTableData(data) {
                ...
                for (var i in res) {
                    output += '<tr ' + 'id="' + 'materialType' + res[i].materialTypeId + '"' + '>' +
                                '<td>' + res[i].materialTypeId + '</td>' +
                                '<td>' + res[i].materialType + '</td>' +
                                '<td class="table-actions-col">' + '<a class="btn btn-primary btn-xs" href="<%= Page.ResolveUrl("~/MaterialTypes/Edit/") %>' + res[i].materialTypeId + '">Edit</a> ' +
                                '<div class="btn btn-danger btn-xs" onclick="OpenDeleteForm(' + res[i].materialTypeId + ')' + '">Delete</div> ' +
            ...
            $(function () {
                ...
                $('#filter-btn').click(function () {
                    ...
                    newRequestObj.materialTypeId = null;
            ...
            function OpenDeleteForm(elementId) {
                ...
                var newRequestObj = new Object();
                newRequestObj.materialTypeId = elementId;
                var requestDataString = JSON.stringify(newRequestObj);

                bootbox.confirm({
                    title: "Delete material type",
                    message: "Are you sure you want to delete the material type?",
                    buttons: {
                        cancel: {
                            label: 'Cancel'
                        },
                    ...
                    callback: function (result) {
                        if (result) {
                            $.ajax({
                                type: "POST",
                                url: "Default.aspx/DeleteMaterialType",
                                async: true,
                                contentType: "application/json; charset=utf-8",
                                dataType: "json",
                                data: requestDataString,
